...
final class AppController {
    ...
    AppView handle(String input) {
        ...
        if (input.startsWith("search")){
            ...
            List<String> textSplits = Arrays.asList(splits).subList(1,splits.length);
            String nameToSearch = String.join("", textSplits);
        ...
        if (input.startsWith("show")){
            ...
            if (personList==null||personList.size()!=1){
                return new AppView("The search did not return exactly one person." ,commands);
            }

            StringBuilder sb = new StringBuilder();
            Person person = personList.get(0);
            String description = person.toString().replace("null","");
            String[] fileds = description.split(", ");
